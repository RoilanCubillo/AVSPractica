...
namespace CentralAdmin.Controllers.Inventario
{
    ...
    public class ArticulosController : Controller
    {
        ...
        public ActionResult GetDynamicJson(int[] families, int[] departments, int[] categories, int[] subCategories, int[] segments)
        {
            ...
            List<EN_Item> list = _objService.GetAllItemsDynamicList(searchValue, orderColumn, sortColumnDir, skip, take
                     , families?.ToList() ?? new List<int>(), departments?.ToList() ?? new List<int>()
                     , categories?.ToList() ?? new List<int>(), subCategories?.ToList() ?? new List<int>()
                     , segments?.ToList() ?? new List<int>(), StoreAuthorizationAttribute.StoresAvailable(Session)).ToList();

            // Obtención de cantidades totales de base de datos y cantidad de registros filtrados
            Dictionary<string, int> data = _objService.GetItemsCountRecords(searchValue, families?.ToList() ?? new List<int>(), departments?.ToList() ?? new List<int>()
               , categories?.ToList() ?? new List<int>(), subCategories?.ToList() ?? new List<int>()
               , segments?.ToList() ?? new List<int>(), StoreAuthorizationAttribute.StoresAvailable(Session));
