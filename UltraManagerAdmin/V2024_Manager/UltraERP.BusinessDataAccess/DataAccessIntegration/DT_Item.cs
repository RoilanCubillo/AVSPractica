...
namespace UltraERP.BusinessDataAccess.DataAccessIntegration
{
    public class DT_Item : DT
    {
        ...
        {
            List<EN_ItemStore> lista = new List<EN_ItemStore>();
            var itemStoreList = db.UEP_ITEMSTORE_GET_BY_ITEMID(itemID, storesID).ToList();
            EN_ItemStore itemStore = new EN_ItemStore();
            itemStore.ItemID = itemStoreList[0].ItemID;
            itemStore.StoreID = itemStoreList[0].StoreID ?? 0;
            itemStore.SupplierID = itemStoreList[0].SupplierID ?? 0;
            itemStore.StoreName = itemStoreList[0].StoreName;
            itemStore.SupplierName = itemStoreList[0].SupplierName;
            itemStore.Quantity = Convert.ToDecimal(itemStoreList[0].Quantity.ToString());
            itemStore.Cost = Convert.ToDecimal(itemStoreList[0].Cost.ToString());
            itemStore.GrossCost = Convert.ToDecimal(itemStoreList[0].GrossCost.ToString());
            itemStore.Utility = Convert.ToDecimal(itemStoreList[0].Utility.ToString());
            itemStore.CustomerDiscount = Convert.ToDecimal(itemStoreList[0].CustomerDiscount.ToString());
            itemStore.InvoiceDiscount = Convert.ToDecimal(itemStoreList[0].InvoiceDiscount.ToString());
            itemStore.LastReceived = Convert.ToDateTime(itemStoreList[0].LastReceived.ToString());
            lista.Add(itemStore);
            //.Select(i => new EN_ItemStore
            //{
            //    ItemID = i.ItemID ?? 0,
            //    StoreID = i.StoreID ?? 0,
            ...
            //    InvoiceDiscount = i.InvoiceDiscount ?? 0,
            //    LastReceived = i.LastReceived ?? new DateTime(1900, 1, 1)
            //})
            //.ToList();
